...
const generateTemperatureHistory = (baseTemp: number) => {
  ...
  for (let i = 23; i >= 0; i--) {
    history.push({
      ...
      value: baseTemp + (Math.random() - 0.5) * 1.5,
    });
  }
  return history;
};
...
const generateHeartRateHistory = (baseRate: number) => {
  ...
  for (let i = 23; i >= 0; i--) {
    history.push({
      timestamp: Date.now() - (i * 3600 * 1000),
      value: baseRate + (Math.random() - 0.5) * 15,
    });
  }
  return history;
};
...
const generateActivityHistory = () => {
  const activity = [];
  for (let i = 0; i < 24; i++) {
    activity.push({ hour: i, activity: Math.floor(Math.random() * 80) });
  }
  return activity;
};
...
export const getHerdSummary = (): HerdSummary => {
  ...
  const averageRssi = Math.round(processedCattleData.reduce((sum, cow) => sum + cow.rssi, 0) / processedCattleData.length);
